...
const Notifications = () => {
  ...
  const currentNotifications = notificationList.slice(startIndex, endIndex);

  // Reset to first page when activeType changes
  useEffect(() => {
    setCurrentPage(1);
  }, [activeType]);
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-white">
              <div className="flex items-center text-sm text-gray-700">
                <span>
                  Showing {startIndex + 1} to {Math.min(endIndex, notificationList.length)} of {notificationList.length} results
                </span>
              </div>
